...
between ::
  ...
  -> Parser a
  -> Parser a
between l r c = l >>> c >>= \v -> r >>> return v
...
hex ::
  Parser Char
hex = thisMany 4 (digit ||| satisfy (\x-> x>='a' && x<='f' )) >>= \ls -> case readHex ls of
  Empty  -> failed
  Full v -> return (chr v)
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = pa >>= \a -> list (ps >>> pa) >>= \as -> return (a :. as)
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby pa ps = sepby1 pa ps ||| return Nil
...
eof ::
  Parser ()
eof = P $ \inp -> if isEmpty inp then Result Nil () else ErrorResult (ExpectedEof inp)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ls = satisfy (and . sequence ls)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ls = satisfy (or . sequence ls)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r c = betweenCharTok l r $ c `sepby` charTok ','
